...
def store_signal_in_db(result):
    try:
        ...
        # Ensure table exists
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS signals_nse_stock_fno (
        ...
        cursor.execute(
            "SELECT COUNT(*) FROM signals_nse_stock_fno WHERE symbol = ? AND signal_time = ?",
        ...
        if exists == 0:
            cursor.execute('''
                INSERT INTO signals_nse_stock_fno (
            ...
            for symbol in instruments:
                ...
                try:
                    candles = kite.historical_data(
                        ...
                        from_date=from_date,
                        to_date=to_date,
                        interval=nse_fno_interval,  # ⏱ test mode
                        continuous=False
                    )
